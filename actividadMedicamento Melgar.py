...
def eliminarmedicamento():
     ...
     if seleccionado:
         ...
         if messagebox.askyesno("Eliminar medicamento", f"¿Esta seguro de eliminar este medicamento'{treeview.item(id_item,'values')[0]}'?"):
             del medicamentos_data[indice]
             guardar_en_archivo() #guardar los cambios en el archivo
             cargar_treeview()
             messagebox.showinfo("Eliminar medicamento","medicamento eliminado exitosamente >:P")
     else:
        messagebox.showwarning("Eliminar medicamento","No se ha seleccionado ningun medicamento ( ._. )")
        return
